...
function testMatch(testValue, operator, nbsTab) {
  ...
  switch(operator) {
    ...
    case 'fois':
      ...
      break;
    case 'concat':
      nb = parseInt(nbs[0].toString() + nbs[1].toString());
      break;
    default:
  ...
  if (nbs.length === 1) {
    ...
    return testMatch(testValue, 'plus', nbs) || testMatch(testValue, 'fois', nbs) || testMatch(testValue, 'concat', nbs);
  }
}
...
for (let line of input) {
    ...
    if (testMatch(testValue, 'plus', nbsTab) || testMatch(testValue, 'fois', nbsTab) || testMatch(testValue, 'concat', nbsTab)) {
